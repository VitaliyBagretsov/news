...
выбирает из неё записи с `isActive=true`, а технические настройки извлечения
HTML получает из связанной записи `news.media_parser_config`. Если
`PARSER_RUN_ON_START=true`, первый сбор начинается сразу после готовности
приложения. Далее Nest scheduler запускает сбор каждые пять минут.
...
Сбор разделён на три независимых backend-модуля:

- `html-reader` выполняет HTTP-запрос и возвращает HTML;
- `html-parser` применяет CSS-селекторы и безопасные именованные стратегии;
- `news-collector` проверяет дубли и сохраняет результат в транзакции.

В БД не хранится исполняемый JavaScript. Для внешних идентификаторов и форматов
дат сохраняются только разрешённые ключи стратегий. Администратор открывает
настройку отдельной кнопкой «Парсер» на карточке медиа. Страница позволяет
изменить конфигурацию и выполнить preview без сохранения новости.

Для существующей локальной БД примените идемпотентное обновление схемы:

```bash
npm run db:migrate
```

Production deployment запускает это обновление автоматически до перезапуска
backend. Начальные настройки RT, РИА и CNews добавляются только при их отсутствии.
...
необязательные сведения о редакции и возвращает созданную запись `news.media`.
...
Логотип media загружается отдельно от JSON-формы через multipart endpoint
`POST /api/media/:id/logo`; удалить его можно через `DELETE /api/media/:id/logo`.
Обе операции доступны только роли `admin`. Поддерживаются PNG, JPEG и WebP до
2 MiB, при этом backend проверяет MIME и сигнатуру содержимого. В колонке
`news.media.logo` хранится только сгенерированное имя файла, а само изображение
доступно через `GET /api/media/images/:fileName`.

Локально файлы находятся в `backend/public/images`, а Docker использует путь
`/news/public/images` и именованный volume `news_media_images`. Поэтому
изображения не входят во frontend-образ и не исчезают при следующем deployment.
При отсутствии или ошибке загрузки frontend показывает локальный placeholder и
не выполняет запрос к несуществующему `/media.png`.
