...
Production-образ собирается многостадийным `Dockerfile`: Node.js компилирует
приложение, после чего Nginx раздаёт только статические файлы. Сборка рассчитана
на публикацию приложения по пути `/news/`, а запросы API направляются на
`/news/api`.

Внутри контейнера endpoint `/healthz` используется Docker healthcheck. Доступ
из интернета предоставляет общий reverse proxy, поэтому frontend-контейнер не
публикует отдельный порт ВМ.
